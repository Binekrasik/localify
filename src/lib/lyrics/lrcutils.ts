export function getTimestampTag(time: number): string {
    ...
    return `${time} / [${Math.floor(time / 60).toString().padStart(2, '0')}:${Math.floor(time % 60).toString().padStart(2, '0')}.${(time % 60 % 1 * 100).toString().slice(0, 2).replaceAll('.', '').padStart(2, '0')}]`
}
